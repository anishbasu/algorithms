...
for j in xrange(1,len(numbers)):#For j in range 1 to length of numbers
    ...
    while i >= 0 and numbers[i] > key:#Move i to the head of the list until key is greater than the current number.
        numbers[i+1] = numbers[i]#Push the number back one slot because it's greater than the key
